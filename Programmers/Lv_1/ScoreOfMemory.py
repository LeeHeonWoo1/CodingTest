...
<입출력 예시>
  - 입력:
    name = ["may", "kein", "kain", "radi"], yearning = [5, 10, 1, 3], photo = [["may", "kein", "kain", "radi"],["may", "kein", "brin", "deny"], ["kon", "kain", "may", "coni"]]
...
def solution(name, yearning, photo):
    answer = [] # 결과가 담길 빈 리스트
    for name_list in photo: # photo 배열 안의 각 배열을 순회하면서
        asumption = 0 # 총 점수를 매길 변수 생성
        for i, person in enumerate(name): # 각 사람들에 대해서
            if person in name_list: # photo 배열 안의 각 배열에 name의 요소가 들어있다면
                asumption += yearning[i] # 결과 도출 변수에 점수 더하기
            else: # 아니라면
                continue # 건너뛰기
        answer.append(asumption) # 빈 리스트에 결과 추가
